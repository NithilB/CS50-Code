...
bool print_winner(void)
{
    ...
    for (int a = 0; a < candidate_count; a++){
        if (candidates[a].votes > (voter_count*0.5)){
            printf("%s\n", candidates[a].name);
            return true;
        }
...
bool is_tie(int min)
{
    ...
    for(int t = 0; t < candidate_count; t++){
        if(min != candidates[t].votes && candidates[t].eliminated == false){ 
            return false;
        }
    }
    return true;
...
void eliminate(int min)
{
    ...
    for(int e = 0; e < candidate_count; e++){
        ...
        } else if (candidates[e].eliminated == true) { 
            candidates[e].eliminated = true;
        } else {
            candidates[e].eliminated = false;
        }
    } 
